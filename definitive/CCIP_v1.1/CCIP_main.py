...
def load_params(model, gender, age, weight, height):
    ...
    if model== 'Schnider, propofol':
        ...
        par= minto_params_calc(gender,age,weight,height)
        
    elif model== 'Gepts, sufentanil':
...
with open('init_params.txt','r') as ff:
    row= ff.readline()
row= row.split(',')
model= str(row[0]+','+row[1])
age= float(row[2])
weight= float(row[3])
height= float(row[4])
gender= row[5]
...
params= load_params(model, gender, age, weight, height)
...
def update(frame):
    ...
    global I, I_log, t, t_counter, newCet, paused
